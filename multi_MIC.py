...
class pre(object):
	...
	def load_data(self):
		with open(self.path + 'data/data_MIC_2D_all_smiles.json', 'r') as f:
			self.mols = json.load(f)
		mol = []
		for i in range(len(self.mols)):
			atom = []
			for j in range(len(self.mols[i])):
				atom.append(self.mols[i][j][:3])
			mol.append(atom)
		self.mols = mol
		with open(self.path + 'data/data_MIC_2D_all_labels.json', 'r') as f:
	...
	def correction(self):
		...
		for i in range(len(self.mols)):
			...
			dx = max_x - min_x
			dy = max_y - min_y
			if dx > 25.6 or dy > 25.6:
				continue
			for j in range(len(mol)):
				coor_xy[j][0] -= (min_x + 0.5 * dx - 0.5 * 24)
				coor_xy[j][1] -= (min_y + 0.5 * dy - 0.5 * 24)
			ppp = []
			pp = []
...
def pre_main(path='/public/home/pcoss1/zh/multi_MIC/'):
	main = pre(path)
	#main.load_data()
	#main.longest_distance()
	#main.correction()
	#main.shuffle()
	with open(main.path + 'data/train_mols.json', 'r') as f:
		main.train_mols = json.load(f)
	with open(main.path + 'data/test_mols.json', 'r') as f:
		main.test_mols = json.load(f)
	with open(main.path + 'data/train_pre02.json', 'r') as f:
		main.train_pre02 = json.load(f)
	with open(main.path + 'data/train_pre23.json', 'r') as f:
		main.train_pre23 = json.load(f)
	with open(main.path + 'data/train_pre34.json', 'r') as f:
		main.train_pre34 = json.load(f)
	main.image_pre(_train=False, _test=True)
	main.image_drawer()
...
class trainer(object):
	...
	def model_build(self):
		...
		x0 = BN()(x0)
		predictions = Dense(self.out,activation='sigmoid')(x0)
...
if __name__ == '__main__':
	pre_main()
	main = trainer(path='/public/home/pcoss1/zh/multi_MIC/', lr=0.001, batch_size=128, epochs=1000, out=1, _type='classify', class_weight=None)
	main.main_trainer()
